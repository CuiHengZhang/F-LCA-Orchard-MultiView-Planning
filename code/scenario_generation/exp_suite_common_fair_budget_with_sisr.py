...
def run_aco(seed: int, n_ants: int = 20, n_iter: int = 1500, candidate_k: int = 8, return_history: bool = False):
    """
    Fair ACO baseline:
...
# ---------------------------- SISR-VRP Baseline ----------------------------
# A lightweight VRP-oriented ruin-and-recreate baseline inspired by
# Slack Induction by String Removals (SISR) for vehicle routing problems.
#
# This implementation is intentionally a controllable baseline for this paper:
# - it removes contiguous route strings rather than arbitrary large task sets;
# - it repairs removed tasks using greedy insertion with a light load penalty;
# - it uses only limited intra-route 2-opt refinement;
# - it does not use F-LCA-specific mechanisms such as energy-aware partitioning,
#   boundary exchange, layered refinement, or path boost.
#
# It uses the same ExperimentProblem, observation model, evaluator, seeds, and
# CSV result format as GA/PSO/ACO/F-LCA.


def _sisr_tree_xy(problem: ExperimentProblem, tid: int):
    return problem.trees[int(tid)]


def _sisr_segment_cost(problem: ExperimentProblem, p1, p2) -> float:
    d = math.hypot(float(p1[0]) - float(p2[0]), float(p1[1]) - float(p2[1]))
    obs = problem.env.segment_obstacle_penalty(p1, p2)
    return float(d + problem.alg_cfg.lambda_obstacle * obs)


def _sisr_route_edge_cost(problem: ExperimentProblem, vid: int, order: List[int], idx: int) -> float:
    """Local edge contribution around position idx in a route."""
    if not order:
        return 0.0
    curr = _sisr_tree_xy(problem, order[idx])
    prev_p = problem.env.start_positions[int(vid)] if idx == 0 else _sisr_tree_xy(problem, order[idx - 1])
    if idx + 1 < len(order):
        next_p = _sisr_tree_xy(problem, order[idx + 1])
        return _sisr_segment_cost(problem, prev_p, curr) + _sisr_segment_cost(problem, curr, next_p)
    return _sisr_segment_cost(problem, prev_p, curr)


def _sisr_nearest_start_partition(problem: ExperimentProblem) -> Dict[int, List[int]]:
    groups = {vid: [] for vid in range(problem.vehicle_num)}
    for tid, (x, y) in enumerate(problem.trees):
        vid = min(
            range(problem.vehicle_num),
            key=lambda v: math.hypot(x - problem.env.start_positions[v][0], y - problem.env.start_positions[v][1]),
        )
        groups[int(vid)].append(int(tid))
    return problem.repair_groups(groups)


def _sisr_nearest_neighbor_order(problem: ExperimentProblem, vid: int, tids: List[int]) -> List[int]:
    remaining = set(int(t) for t in tids)
    order: List[int] = []
    curr = problem.env.start_positions[int(vid)]
    while remaining:
        best = min(
            remaining,
            key=lambda t: _sisr_segment_cost(problem, curr, _sisr_tree_xy(problem, t))
                          + 0.05 * problem.env.obstacle_pressure(_sisr_tree_xy(problem, t)),
        )
        order.append(int(best))
        curr = _sisr_tree_xy(problem, int(best))
        remaining.remove(int(best))
    return order


def _sisr_initial_solution(problem: ExperimentProblem):
    groups = _sisr_nearest_start_partition(problem)
    orders = {vid: _sisr_nearest_neighbor_order(problem, vid, groups.get(vid, [])) for vid in range(problem.vehicle_num)}
    return problem.repair_groups(groups), problem.repair_orders(groups, orders)


def _sisr_remove_string_from_route(
    ...
    """Remove one contiguous string from one route and return removed tree IDs."""
    non_empty = [vid for vid in range(problem.vehicle_num) if len(orders.get(vid, [])) >= 2]
    if not non_empty:
        return []

    if route_bias == "long":
        # Prefer routes with larger simple centerline length.
        def route_len(v):
            order = orders.get(v, [])
            if not order:
                return 0.0
            curr = problem.env.start_positions[int(v)]
            total = 0.0
            for tid in order:
                p = _sisr_tree_xy(problem, tid)
                total += _sisr_segment_cost(problem, curr, p)
                curr = p
            return total
        weights = np.array([route_len(v) for v in non_empty], dtype=float)
        if float(weights.sum()) <= 1e-12:
            vid = int(rng.choice(non_empty))
        else:
            probs = weights / float(weights.sum())
            vid = int(non_empty[int(rng.choice(len(non_empty), p=probs))])
    else:
        vid = int(rng.choice(non_empty))

    route = orders.get(vid, [])
    if len(route) <= 1:
        return []

    max_use_len = min(max_len, max(1, len(route) - 1))
    min_use_len = min(min_len, max_use_len)
    string_len = int(rng.integers(min_use_len, max_use_len + 1))

    if route_bias == "expensive" and len(route) > string_len:
        edge_costs = np.array([_sisr_route_edge_cost(problem, vid, route, i) for i in range(len(route))], dtype=float)
        # Select a center position with probability proportional to local edge cost.
        if float(edge_costs.sum()) <= 1e-12:
            center = int(rng.integers(len(route)))
        else:
            center = int(rng.choice(len(route), p=edge_costs / float(edge_costs.sum())))
        start_idx = int(max(0, min(len(route) - string_len, center - string_len // 2)))
    else:
        start_idx = int(rng.integers(0, len(route) - string_len + 1))

    removed = [int(t) for t in route[start_idx:start_idx + string_len]]
    rem_set = set(removed)
    orders[vid] = [int(t) for t in route if int(t) not in rem_set]
    groups[vid] = [int(t) for t in groups.get(vid, []) if int(t) not in rem_set]
    return removed


def _sisr_string_removal(
    problem: ExperimentProblem,
    rng: np.random.Generator,
    groups,
    orders,
    ...
    """SISR-style removal: remove a small number of contiguous strings."""
    removed: List[int] = []
    n_strings = int(rng.integers(1, max_strings + 1))
    for _ in range(n_strings):
        ...
        if r < 0.40:
            ...
            bias = "random"
        part = _sisr_remove_string_from_route(problem, rng, groups, orders, bias, min_string_len, max_string_len)
        removed.extend(part)
    return list(dict.fromkeys(int(t) for t in removed))


def _sisr_candidate_positions(rng: np.random.Generator, order: List[int]) -> List[int]:
    n = len(order)
    if n <= 12:
        return list(range(n + 1))
    pos = {0, n}
    # sample a modest number of candidate insert positions to control strength and runtime
    sample_n = min(8, n - 1)
    if sample_n > 0:
        pos.update(int(x) for x in rng.choice(np.arange(1, n), size=sample_n, replace=False).tolist())
    return sorted(pos)


def _sisr_insertion_delta(problem: ExperimentProblem, orders, vid: int, tid: int, pos: int) -> float:
    order = orders.get(int(vid), [])
    prev_p = problem.env.start_positions[int(vid)] if pos == 0 else _sisr_tree_xy(problem, order[pos - 1])
    tid_p = _sisr_tree_xy(problem, int(tid))
    if pos >= len(order):
        return _sisr_segment_cost(problem, prev_p, tid_p)
    next_p = _sisr_tree_xy(problem, order[pos])
    return (
        _sisr_segment_cost(problem, prev_p, tid_p)
        + _sisr_segment_cost(problem, tid_p, next_p)
        - _sisr_segment_cost(problem, prev_p, next_p)
    )


def _sisr_best_insert(problem: ExperimentProblem, rng: np.random.Generator, groups, orders, tid: int):
    """Greedy insertion with light count-balance penalty."""
    best = None
    counts = np.array([len(groups.get(v, [])) for v in range(problem.vehicle_num)], dtype=float)
    mean_count = float(np.mean(counts)) if len(counts) else 0.0
    for vid in range(problem.vehicle_num):
        order = orders.get(vid, [])
        # Light penalty only; this is not F-LCA's energy-aware partitioning.
        count_penalty = 3.0 * max(0.0, (counts[vid] + 1.0) - mean_count - 5.0)
        for pos in _sisr_candidate_positions(rng, order):
            delta = _sisr_insertion_delta(problem, orders, vid, tid, pos) + count_penalty
            if best is None or delta < best[0]:
                best = (float(delta), int(vid), int(pos))
    assert best is not None
    return best


def _sisr_greedy_repair(problem: ExperimentProblem, rng: np.random.Generator, groups, orders, removed: List[int]):
    rem = [int(t) for t in removed]
    # Reinsert far/expensive nodes earlier to avoid leaving difficult nodes to the end.
    if rng.random() < 0.60:
        rem.sort(
            key=lambda t: problem.env.obstacle_pressure(_sisr_tree_xy(problem, t))
                          + 0.01 * min(
                              math.hypot(_sisr_tree_xy(problem, t)[0] - s[0], _sisr_tree_xy(problem, t)[1] - s[1])
                              for s in problem.env.start_positions
                          ),
            reverse=True,
        )
    else:
        rng.shuffle(rem)
    for tid in rem:
        _, vid, pos = _sisr_best_insert(problem, rng, groups, orders, tid)
        groups[int(vid)].append(int(tid))
        orders[int(vid)].insert(int(pos), int(tid))
    groups = problem.repair_groups(groups)
    orders = problem.repair_orders(groups, orders)
    return groups, orders


def _sisr_limited_two_opt(problem: ExperimentProblem, rng: np.random.Generator, groups, orders, max_trials: int = 10):
    """Limited intra-route 2-opt; no inter-route relocate/swap."""
    best_groups = deepcopy(groups)
    best_orders = deepcopy(orders)
    best_metrics = problem.evaluate(best_groups, best_orders)
    best_cost = problem.scalar_cost(best_metrics)
    for _ in range(max_trials):
        vid = int(rng.integers(problem.vehicle_num))
        if len(best_orders.get(vid, [])) < 4:
            continue
        cand_groups = deepcopy(best_groups)
        cand_orders = deepcopy(best_orders)
        i, j = sorted(rng.choice(len(cand_orders[vid]), size=2, replace=False).tolist())
        if j <= i:
            continue
        cand_orders[vid][i:j + 1] = list(reversed(cand_orders[vid][i:j + 1]))
        cand_orders = problem.repair_orders(cand_groups, cand_orders)
        ...
        if cand_cost + 1e-9 < best_cost:
            best_groups, best_orders, best_metrics, best_cost = cand_groups, cand_orders, cand_metrics, cand_cost
    return best_groups, best_orders, best_metrics
...
def run_sisr_vrp(
    ...
    Purpose:
    - recent VRP-oriented ruin-and-recreate baseline;
    - removes contiguous strings from routes and greedily reinserts them;
    - same environment / observation model / evaluator as F-LCA and baselines;
    - no adaptive operator weights, regret repair, high-performance HGS solver,
      battery-aware partitioning, or F-LCA-specific refinements.
    """
    problem = ExperimentProblem(seed)
    rng = np.random.default_rng(seed)
    ...
    for it in range(n_iter):
        ...
        if not removed:
            history.append(float(best_metrics["primary_cost"]))
            continue
        cand_groups, cand_orders = _sisr_greedy_repair(problem, rng, cand_groups, cand_orders, removed)
        if (it + 1) % local_search_every == 0:
            cand_groups, cand_orders, cand_metrics = _sisr_limited_two_opt(problem, rng, cand_groups, cand_orders, max_trials=8)
        else:
            cand_metrics = problem.evaluate(cand_groups, cand_orders)
        cand_cost = problem.scalar_cost(cand_metrics)
