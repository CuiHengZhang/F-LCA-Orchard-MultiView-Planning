from exp_suite_common_fair_budget_with_sisr import (
    SEEDS,
    run_sisr_vrp,
    save_results_bundle,
    print_brief,
    print_saved_paths,
)

OUT_DIR = "sisr_vrp_20seed_fair_budget"
MAX_RUNS = len(SEEDS)  # 可先改成 5 试跑前五组


def main():
    rows = []
    print("\n===== SISR-VRP (fair budget) =====")
    use_seeds = SEEDS[:MAX_RUNS]
    total_runs = len(use_seeds)
    for idx, seed in enumerate(use_seeds, start=1):
        row, _ = run_sisr_vrp(seed)
        rows.append(row)
        print(
            f"run={idx}/{total_runs} | seed={seed} | "
            f"primary={row['primary_cost']:.2f} | "
            f"distance={row['total_distance']:.2f} | "
            f"obstacle={row['obstacle_penalty']:.2f} | "
            f"energy={row['energy_range_percent']:.2f}% | "
            f"time={row['runtime_sec']:.2f}s"
        )

    save_results_bundle(OUT_DIR, rows)
    print("\n===== Summary =====")
    print_brief(rows)
    print_saved_paths(OUT_DIR)


if __name__ == "__main__":
    main()
